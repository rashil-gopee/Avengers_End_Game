...
public class hexmech
{
     ...
     *****************************************************************************/
    public static void fillHex(int i, int j, Hexagon hexagon, Graphics2D g2){
        char c;
        int x = i * (s+t);
        int y = j * h + (i%2) * h/2;
...
//			System.out.println("hex "+(x+r+BORDERS)+" "+(y+r+BORDERS+4));
        }
        else {
            g2.fillPolygon(hex(x,y));
            try {
                g2.setClip(hex(x,y));
                g2.drawImage(hexagon.getPiece().getImage().getScaledInstance(60, 60, Image.SCALE_DEFAULT), x, y, null);
            }
            catch (Exception e){
                e.printStackTrace();
            }
    ...
    public static void modelDraw(int i,int j,String info,Graphics2D g2){
        int x = i * (s+t);
        int y = j * h + (i%2) * h/2;
        g2.fillPolygon(hex(x,y));
        try {
        ...
        g2.setColor(AvengersGameView.COLOURTWOTXT);
        g2.drawString(info, x+r+BORDERS, y+r+BORDERS+4); //FIXME han
